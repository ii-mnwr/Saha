...
const BlueSkeleton = styled(Skeleton)(({ theme }) => ({
  ...
  '&::after': {
    background:
      'linear-gradient(90deg, rgba(0, 123, 255, 0.1) 25%, rgba(0, 123, 255, 0.2) 50%, rgba(0, 123, 255, 0.1) 75%)', // Gradient matching the theme colors
  },
  // animation: `${diagonalAnimation} 1.5s linear infinite`,
}));

export default function SkeletonHorizontalItem({ ...other }: CardProps) {
  return (
    <Card {...other}>
      <BlueSkeleton variant="rectangular" sx={{ paddingTop: '25%', height: 25 }} />
